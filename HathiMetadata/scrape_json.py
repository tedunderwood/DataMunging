...
def extract_subfields(node):
    ...
    if not 'subfields' in node:
        return [' ']
    else:
        listofdicts = node['subfields']
        for sfield in listofdicts:
            for key, value in sfield.items():
...
def subfield_dictionary(field):
    ...
    if not 'subfields' in field:
        return codedictionary
    else:
        subfields = field['subfields']
        for subfield in subfields:
            for key, value in subfield.items():
                if key in codedictionary:
                    codedictionary[key] += " | " + value
                else:
                    codedictionary[key] = value
...
def parse008(field):
    ...
    if len(field) < 39:
        return 'n', '', '', 'xx#', set(), '###'
        # this is unparseable
        # the date, place, and language codes are
        # the appropriate codes for 'no information
        # provided.'
    ...
    language = field[35:38]

    return datetype, date1, date2, place, genres, language
...
def parsemarc(jsonobj):
    ...
    leader = jsonobj['leader']
    # There should only be one leader.
    materialtype = get_materialtype(leader)

    datafields = jsonobj['fields']
    for fielddict in datafields:
        for fieldname, field in fielddict.items():

            if fieldname == "001":
                recordid = field.replace("MIU01-", "")

            if fieldname == "008":
                datetype, date1, date2, place, genreset, language = parse008(field)

            if fieldname == '245':
                fieldstoget = ['a', 'b', 'c', 'f', 'g', 'k', 'n', 'p']
                titlefields = subfield_dictionary(field)
                titleterms = []
                for field in fieldstoget:
                    if field in titlefields:
                        term = titlefields[field]
                        if field != 'a' and field != 'b':
                            term = "$" + field + ": " + term
                        titleterms.append(term)

                title = ' | '.join(titleterms)
            ...
            elif fieldname == "655" or fieldname == "155":
            ...
            elif fieldname.isnumeric() and int(fieldname) >= 600 and int(fieldname) <= 699:
    ...
    return HTid, recordid, author, authordate, title, date, LOCnum, imprint, enumcron, OCLC, subjset, genreset, materialtype, datetype, date1, date2, place, contents, geographicset, language

# MAIN SCRIPT BEGINS HERE:

sourcedir = '/data/private/metadata/json/'
sourcefile = 'meta_ic.json'
source = os.path.join(sourcedir, sourcefile)
limit_for_run = 2000000

# This is the name of the output file we're going to create
outpath = '/homed/tunder/meta/meta_ic_new1.tsv'

errorpath = '/homed/tunder/meta/parse_errors.txt'
...
if os.path.isfile(outpath):
    ...
    with open(outpath, mode = 'w', encoding = 'utf-8') as f:
        f.write('docid\trecordid\toclc\tlocnum\tauthor\tauthordate\timprint\tdatetype\tstartdate\tenddate\timprintdate\tplace\tenumcron\tmaterialtype\tlanguage\tsubjects\tgenres\tgeographics\tcontents\ttitle\n')

progresspath = '/homed/tunder/meta/lastprocessed_new.txt'
lastline = 0
...
print("Reading from line " + str(lastline) + " to line " + str(line2endat))
errors = []

def writestuff(progresspath, outpath, ctr, lines2write):
...
with open(source, encoding = 'utf-8') as f:
    for jsonstring in f:
        ...
        if (ctr % 1000) == 1:
            # We write accumulated lines, and record our position in the file.
            writestuff(progresspath, outpath, ctr, lines2write)
            lines2write = []
        ...
        try:
            HTid, recordid, author, authordate, title, textdate, LOCnum, imprint, enumcron, OCLC, subjset, genreset, materialtype, datetype, date1, date2, place, contents, geographicset, language = parsemarc(jsonobj)
        except:
            print('parse error')
            errors.append(jsonstring)
            continue

        if len(HTid) < 1:
            print('Empty Hathi vol ID. Skipped.')
        ...
        writestring = HTid + '\t' + recordid + '\t' + OCLC + '\t' + LOCnum + '\t' + author + '\t' + authordate + '\t' + imprint + '\t' + datetype + '\t' + date1 + '\t' + date2  + '\t' + textdate + '\t' + place + '\t' + enumcron + '\t' + materialtype + '\t' + language + '\t' + subjstr + '\t' + genrestr  +'\t' + geogstr + '\t' + contents + '\t' + title
        writestring = writestring.replace('\n', '') + '\n'

        lines2write.append(writestring)

# The loop is done, but there might still be some work to write.
writestuff(progresspath, outpath, ctr, lines2write)

# we should also record some of the parse errors so we know
# what went wrong
